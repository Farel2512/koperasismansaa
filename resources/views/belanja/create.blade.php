...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="container-fluid">
            <form id="belanja-form" action="{{ route($rolePrefix . '.belanja.store') }}" method="POST">
                @csrf
                <div class="form-group">
                    <label for="no_belanja">No Belanja</label>
                    <input type="text" name="no_belanja" id="no_belanja" class="form-control" required readonly>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="barang_id" class="form-control">
                        ...
                        @foreach ($barangs as $barang)
                            <option value="{{ $barang->id }}" data-harga="{{ $barang->harga_beli }}" data-stok="{{ $barang->stok }}">{{ $barang->nama }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        function generateTransactionNumber() {
            ...
            return 'KB-00' + randomNum;
        }

        document.getElementById('no_belanja').value = generateTransactionNumber();
    });
    ...
    document.getElementById('belanja-form').addEventListener('submit', function(event) {
        const totalPrice = parseInt(totalPriceElem.textContent.replace('Rp. ', '').replace(/\./g, '')) || 0;
        const uangBayar = parseInt(uangBayarInput.value) || 0;
